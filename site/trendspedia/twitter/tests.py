...
class TwitterView(django.test.TestCase):
    ...
    # TODO: get login workings
##    def test_login_home_twitter(self):
##        # Create an instance of a GET request.
##        response = self.client.post('/login', {'username': 'cinternetobservatory@gmail.com', 'password': 'ilovecs3282'})
...
##        # cinternetobservatory@gmail.com
##        # ilovecs3282
##    def test_addTask(self):
##        response = views'/twitter/addtask/search', {'q': 'ocaml', 'pageID': 1234})
##        print response
##        self.assertEqual(response.status_code, 200)
        
    def test_search(self):
        response = views.search_with_tokens(self.tq.retrieveToken("search"),"search",{'q': 'ocaml', 'pageID': 1234})
        self.assertEqual(response.status_code, 200)
        response = views.search_with_tokens(self.tq.retrieveToken("followersID"),"followersID", {'user_id': 50393960})
        self.assertEqual(response.status_code, 200)
        response = views.search_with_tokens(self.tq.retrieveToken("friendsID"),"friendsID", {'user_id': 50393960})
        self.assertEqual(response.status_code, 200)
        response = views.search_with_tokens(self.tq.retrieveToken("friendshipLookup"),"friendshipLookup", {'user_id': 50393960})
        self.assertEqual(response.status_code, 200)
        response = views.search_with_tokens(self.tq.retrieveToken("followersList"),"followersList", {'screen_name': 'Xecky_hcX', 'cursor':-1})
        self.assertEqual(response.status_code, 200)

    def test_helper_updateWikiArticle(self):
        import helper
        pageID = 1234 # Random ID
        queryResults = {
          "statuses": [
            {
              "coordinates": None,
        ...
        # Tweets Serialization should return greater to or equal to the queryResults returned
        self.assertTrue(len(tweetsSerialization) >= queryResults["search_metadata"]["count"])
