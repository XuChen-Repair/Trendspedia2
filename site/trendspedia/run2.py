...
def processTweets():
    ...
    cnt = 0
    res = tweetCol.find().sort([('createdAt', -1)]).limit(10000)
    for doc in res:
	#print doc
...
class TopEvents:
    ...
    def get_top_K_events(self, tweets, alpha, K, N):
        ...
        #pp.pprint("Frequency distribution for all days is ", self.day_doc_list)
        print self.day_doc_list

        for (day, weight) in top_events:

            freqdist = self.day_doc_list[day]
            print "frequency distribution for day ", day, " = ", freqdist
...
print "result = ", te.get_top_K_events(tweets, 0.5, 500, 10)
